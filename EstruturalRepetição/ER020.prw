...
user function ER020()
    ...
    Do While UPPER(cOpc) == 'S'
        FwAlertInfo("Digite um número inteiro, de 1 a 15.")
        nNumero := Val(FwInputBox("Digite um número inteiro: "))

        if UPPER(cOpc) == 'N'
            lRet := .F.
        elseif nNumero < 1 .or. nNumero > 15
            FwAlertInfo("Por favor, digite um número inteiro positivo no intervalo de 1 a 15.")
        else
        ...
        cOpc := UPPER(FwInputBox("Deseja fazer uma nova operação? S/N"))
    EndDo
